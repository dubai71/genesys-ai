...
if (!context) {
  throw new Error('Unable to detect repository. Run "sinapse init" first.');
}
```
...
```javascript
const { generateImpactSummary } = require('.sinapse-ai/core/code-intel/helpers/devops-helper');
...
### NPM Package with Semantic-Release (sinapse-ai)
```yaml
github:
  pr:
