...
steps:
  ...
  2_check_dependencies:
    ...
    checks:
      ...
      - Does this change affect IDE sync? (.sinapse-ai/infrastructure/scripts/ide-sync/)
      ...
      - Does this change affect other agents? (.sinapse-ai/development/agents/)
...
steps:
  ...
  3_handle_edge_cases:
    ...
    known_pitfalls:
      - Email addresses inside strings may match rename patterns (Issue #159: security@sinapse/sinapse-ai.dev)
  ...
  4_regenerate_manifests:
    ...
    action: |
      ...
      This regenerates .sinapse-ai/install-manifest.yaml
    when: Any file in .sinapse-ai/ or packages/ is created, modified, or deleted

  5_run_ide_sync:
    condition: Changes affect agent definitions or IDE sync system
    action: |
      Run: node .sinapse-ai/infrastructure/scripts/ide-sync/index.js sync --verbose
      Verify all IDEs sync without errors
    when: Changes to .sinapse-ai/development/agents/ or ide-sync/
```
...
- **Pitfall:** `@sinapse/sinapse-ai` inside email `security@sinapse/sinapse-ai.dev` was corrupted
