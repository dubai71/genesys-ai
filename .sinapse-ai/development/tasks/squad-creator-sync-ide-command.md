...
O sistema usa `.sinapse-sync.yaml` na raiz do projeto para configuração:
...
| `Squad alias not found` | Squad não está em squad_aliases | Adicione ao .sinapse-sync.yaml |
| `File exists`          | Destino já existe               | Use --force ou escolha ação |
| `IDE not active`       | IDE não está em active_ides     | Ative no .sinapse-sync.yaml    |
...
const syncConfig = loadYaml('.sinapse-sync.yaml');
