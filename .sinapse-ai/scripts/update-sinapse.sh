#!/bin/bash
# SINAPSE Framework Update - v5.2 (Optimized)
#
# LOGIC:
...
#
# Usage: bash .sinapse-ai/scripts/update-sinapse.sh

set -e

echo "⚡ SINAPSE Update v5.2"
echo ""
...
# Validate: clean working tree for .sinapse-ai
if [ -n "$(git status --porcelain .sinapse-ai/)" ]; then
  echo "❌ Commit .sinapse-ai changes first:"
  git status --short .sinapse-ai/
  echo ""
  echo "Run: git add .sinapse-ai && git commit -m 'your message'"
  exit 1
fi
...
git sparse-checkout set .sinapse-ai
cd - > /dev/null

echo "✅ Fetched upstream"
echo ""

# Build file lists (relative paths without .sinapse-ai/ prefix)
echo "📋 Scanning files..."
find .sinapse-ai -type f | sed 's|^\.sinapse-ai/||' | sort > "$TEMP_DIR/local-files.txt"
find "$TEMP_DIR/upstream/.sinapse-ai" -type f | sed "s|^$TEMP_DIR/upstream/\.sinapse-ai/||" | sort > "$TEMP_DIR/upstream-files.txt"

# Get git-tracked files (for DELETE detection) - ONE call instead of N calls
git ls-files .sinapse-ai | sed 's|^\.sinapse-ai/||' | sort > "$TEMP_DIR/tracked-files.txt"
...
while IFS= read -r rel_path; do
  if ! cmp -s ".sinapse-ai/$rel_path" "$TEMP_DIR/upstream/.sinapse-ai/$rel_path"; then
...
while IFS= read -r rel_path; do
  ...
  cp -a ".sinapse-ai/$rel_path" "$TEMP_DIR/local-only/$rel_path"
...
if [ -s "$REPORT_DELETED" ]; then
  while IFS= read -r rel_path; do
    rm -f ".sinapse-ai/$rel_path"
  done < "$REPORT_DELETED"
fi

# Copy all upstream files (creates new + overwrites existing)
rsync -a "$TEMP_DIR/upstream/.sinapse-ai/" ".sinapse-ai/"

# Restore local-only files
if [ -d "$TEMP_DIR/local-only" ] && [ "$(ls -A "$TEMP_DIR/local-only" 2>/dev/null)" ]; then
  rsync -a "$TEMP_DIR/local-only/" ".sinapse-ai/"
fi

# Clean empty directories
find .sinapse-ai -type d -empty -delete 2>/dev/null || true
...
echo ""
echo "Choose:"
echo "  ✅ Apply:  git add .sinapse-ai && git commit -m 'chore: sync SINAPSE framework'"
echo "  ❌ Cancel: git checkout -- .sinapse-ai/"
echo ""
