 ...
 * Pro Command Module
 *
 * CLI commands for SINAPSE Pro license management and feature gating.
 *
 * Subcommands:
 *   sinapse pro activate --key <KEY>    Activate a license key
 *   sinapse pro status                   Show license status
 *   sinapse pro deactivate               Deactivate the current license
 *   sinapse pro features                 List all pro features
 *   sinapse pro validate                 Force online revalidation
 *   sinapse pro setup                    Configure GitHub Packages access (AC-12)
 *
...
// In framework-dev: __dirname = sinapse-ai/.sinapse-ai/cli/commands/pro → ../../../../pro/license
// In project-dev: pro is installed via npm as @sinapse-fullstack/pro
function resolveLicensePath() {
  ...
  // 2. Try node_modules/@sinapse-fullstack/pro/license (project-dev mode)
  try {
    const proPkg = require.resolve('@sinapse-fullstack/pro/package.json');
    ...
    // @sinapse-fullstack/pro not installed via npm
  }

  // 3. Try project root node_modules (fallback)
  const projectRoot = process.cwd();
  const cwdPath = path.join(projectRoot, 'node_modules', '@sinapse-fullstack', 'pro', 'license');
...
function loadLicenseModules() {
  try {
    ...
    console.error('SINAPSE Pro license module not available.');
    console.error('Install SINAPSE Pro: npm install @sinapse-fullstack/pro');
    process.exit(1);
  }
}

/**
 * Get SINAPSE Core version from package.json
 */
function getSinapseCoreVersion() {
  try {
...
async function activateAction(options) {
  ...
  if (!key) {
    ...
    console.error('Usage: sinapse pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
    process.exit(1);
  }
  ...
  console.log('\nActivating SINAPSE Pro license...');
  console.log(`Key: ${maskKey(key)}`);
  console.log('');

  try {
    const machineId = generateMachineId();
    const sinapseCoreVersion = getSinapseCoreVersion();

    const result = await licenseApi.activate(key, machineId, sinapseCoreVersion);
...
function statusAction() {
  ...
  if (info && info.inGrace) {
    ...
    console.log('  Please revalidate your license: sinapse pro validate');
  }
...
async function deactivateAction(options) {
  ...
  if (!options.force) {
    console.log('\nDeactivating SINAPSE Pro License');
    console.log(`Key: ${maskKey(cache.key)}`);
    console.log('\nThis will:');
  ...
  try {
    ...
    console.log('');
    console.log('To reactivate: sinapse pro activate --key <KEY>');
    console.log('');
...
async function validateAction() {
  ...
  console.log('\nValidating SINAPSE Pro license...\n');
...
// ---------------------------------------------------------------------------
// sinapse pro setup (AC-12: Install-gate)
// ---------------------------------------------------------------------------

/**
 * Setup and verify @sinapse-fullstack/pro installation.
 *
 * Since @sinapse-fullstack/pro is published on the public npm registry,
...
async function setupAction(options) {
  ...
  if (options.verify) {
    // Verify-only mode
    console.log('Verifying @sinapse-fullstack/pro installation...\n');

    try {
      const { execSync } = require('child_process');
      const result = execSync('npm ls @sinapse-fullstack/pro --json', {
        stdio: 'pipe',
        timeout: 15000,
      });
      const parsed = JSON.parse(result.toString());
      const deps = parsed.dependencies || {};
      if (deps['@sinapse-fullstack/pro']) {
        console.log(`✅ @sinapse-fullstack/pro@${deps['@sinapse-fullstack/pro'].version} is installed`);
      } else {
        console.log('❌ @sinapse-fullstack/pro is not installed');
        console.log('');
        console.log('Install with:');
        console.log('  npm install @sinapse-fullstack/pro');
      }
    } catch {
      console.log('❌ @sinapse-fullstack/pro is not installed');
      console.log('');
  ...
  // Install mode
  console.log('@sinapse-fullstack/pro is available on the public npm registry.');
  console.log('No special tokens or configuration needed.\n');

  console.log('Installing @sinapse-fullstack/pro...\n');

  try {
    const { execSync } = require('child_process');
    execSync('npm install @sinapse-fullstack/pro', {
      stdio: 'inherit',
      timeout: 120000,
    });
    console.log('\n✅ @sinapse-fullstack/pro installed successfully!');
  } catch (error) {
  ...
  console.log('  sinapse pro activate --key PRO-XXXX-XXXX-XXXX-XXXX');
  console.log('');
...
/**
 * Create the `sinapse pro` command with all subcommands.
 * @returns {Command}
 */
function createProCommand() {
  const proCmd = new Command('pro')
    .description('SINAPSE Pro license management');
  ...
  proCmd
    .command('setup')
    .description('Install and verify @sinapse-fullstack/pro')
