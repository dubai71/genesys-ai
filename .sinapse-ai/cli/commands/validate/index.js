 ...
 *
 * Usage:
 *   sinapse validate                    # Validate current installation
 *   sinapse validate --repair           # Repair missing/corrupted files
 *   sinapse validate --repair --dry-run # Preview repairs without applying
 *   sinapse validate --detailed         # Show detailed file list
 *   sinapse validate --json             # Output as JSON
 */
...
async function runValidation(options) {
  ...
  if (sourceDir) {
    const sourceManifest = path.join(sourceDir, '.sinapse-ai', 'install-manifest.yaml');
  ...
  if (!sourceDir && options.repair) {
    ...
    const possibleSources = [
      ...
      path.join(projectRoot, 'node_modules/sinapse-ai'),
      path.join(projectRoot, 'node_modules/sinapse-ai'),
    ];

    for (const src of possibleSources) {
      if (fs.existsSync(path.join(src, '.sinapse-ai', 'install-manifest.yaml'))) {
        sourceDir = src;
        break;
      }
    }
